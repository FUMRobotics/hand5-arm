...
void ComputePID(PID *PIDSystem)
{
	...
	if (PIDSystem->ITerm > PIDSystem->I_Max) PIDSystem->ITerm = PIDSystem->I_Max;
	if (PIDSystem->ITerm < PIDSystem->I_Min) PIDSystem->ITerm = PIDSystem->I_Min;
	// ----- Derivative Term
	PIDSystem->DTerm = PIDSystem->Error - PIDSystem->DTerm;    // deltaError
	// ----- Output
	PIDSystem->Out = (PIDSystem->KP * PIDSystem->Error) + (PIDSystem->KI * PIDSystem->ITerm) + (PIDSystem->KD * PIDSystem->DTerm);
	if (PIDSystem->Out > PIDSystem->Out_Max) PIDSystem->Out = PIDSystem->Out_Max;
	if (PIDSystem->Out < PIDSystem->Out_Min) PIDSystem->Out = PIDSystem->Out_Min;
}
...
void ParseTCP(uint8_t *txt)
{
	...
	else if (strcmp(txt, "Fist") == 0) {
		SetMotor(Index_Motor, FINGER_Close, 99);
		SetMotor(Middle_Motor, FINGER_Close, 99);
		SetMotor(Ring_Motor, FINGER_Close, 99);
		SetMotor(Pinky_Motor, FINGER_Close, 99);
	} else if (strcmp(txt, "FingersSpread") == 0) {
